...
from cy_modules.graph cimport Graph
import torch
import torch_sparse
...
cdef class py_PrepareBatchGraph:
    ...
    def SetupTrain(self,idxes,g_list,covered,list actions):
        ...
        for _g in g_list:
            ...
            deref(inner_Graph).node_attributes = _g.node_attributes
    ...
    def edge_index(self):
        matrix = deref(deref(self.inner_PrepareBatchGraph).n2nsum_param)
        edge_index = torch.tensor([matrix.rowIndex, matrix.colIndex], dtype=torch.long)
        return edge_index
    
    @property
    def node_attributes(self):
        _node_attr = deref(self.inner_PrepareBatchGraph).node_attributes
        return torch.tensor(_node_attr)
    ...
    def batch_tensor(self):
        ...
        # Create a tensor filled with -1 (indicating no instance membership)
        #instance_membership = torch.full((matrix.colNum,), -1, dtype=torch.long)
        # Use advanced indexing to fill the tensor
        #colIndex = torch.tensor(matrix.colIndex, dtype=torch.long)
        #rowIndex = torch.tensor(matrix.rowIndex, dtype=torch.long)
        #instance_membership.scatter_(0, colIndex, rowIndex)

        instance_membership = self.subgsum_param['index'][0]
        return instance_membership
