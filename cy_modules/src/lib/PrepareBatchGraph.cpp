                                        ...
                                        const int *actions)
{
    ...
    int num_node_attrs = g_list[0]->node_attributes[0].size();
    node_attributes.clear();
    for (int i = 0; i < (int)idxes.size(); ++i)
    {
        auto g = g_list[idxes[i]];
        ...
        for (int j = 0; j < (int)g->num_nodes; ++j)
        {
            ...
            if (num_node_attrs > 0)
                node_attributes.push_back(g->node_attributes[j]);
            if (!actions)
            {
        ...
        if (actions)
        {
            // make a failed asseess to test
            auto act = actions[idxes[i]];
            if (idx_map[act] < 0)
            {
                std::cout << "PrepareBatchGraph: idx_map[act]: " << idx_map[act] << std::endl;
                exit(0);
            }
            ...
            if(node_cnt + idx_map[act] < 0)
            {
                std::cout << "node_cnt: " << node_cnt << ", idx_map[act]: " << idx_map[act] << std::endl;
                exit(0);
            }
