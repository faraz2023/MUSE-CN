...
def dglblock_to_pyg_custom(dgl_block, node_feature_key='feat', edge_feature_key='edge_feat', device='cpu'):

    src, dst = dgl_block.edges()
    edge_index = torch.stack([src, dst], dim=0).to(device)  # Shape [2, num_edges]

    node_features = dgl_block.ndata[node_feature_key]["_N"].to(device) if node_feature_key in dgl_block.ndata else None
    edge_features = dgl_block.edata[edge_feature_key].to(device) if edge_feature_key in dgl_block.edata else None
    
    data = {
        'edge_index': edge_index,
        'edge_attr': edge_features,
        'node_input': node_features,
        'batch': torch.zeros(node_features.shape[0], dtype=torch.int64).to(device),
    }
...
class Universal_Collator(object):
    def __init__(self, g, use_saint, batch_size, device, tasks,
                ...
                link_negative_ratio, \
                decor_size, der, dfr, \
                recon_size, recon_mask_rate, num_layers=3,
                ):
        ...
        if 'p_link' in tasks:
            # link
            multi_layer_l = [15, 10, 5]
            if(num_layers > 3):
                for i in range(num_layers-3):
                    multi_layer_l.append(5)
            sampler = dgl.dataloading.MultiLayerNeighborSampler(multi_layer_l)
    ...
    def __call__(self, indices):
        ...
        if 'p_link' in self.tasks:
            ...
            sg , pos_u, pos_v, neg_u, neg_v = link_prediction_data_process(self.link_sampler.sample(self.g, indices), self.device)
            sg = [dglblock_to_pyg_custom(_item, device=self.device) for _item in sg]
            outputs['p_link'] = [sg, pos_u, pos_v, neg_u, neg_v]
