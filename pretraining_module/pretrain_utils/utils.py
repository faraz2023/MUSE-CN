...
def load(model, dir_path, opt, reset_params=False):
    ...
    MEGA_config = json.load(open(epoch_path + '/MEGA_config.json'))
...
def save(model, optimizer, scheduler, step, opt, dir_path, name):
    ...
    with open('{}/MODEL_config.json'.format(epoch_path), 'w') as fp: json.dump(opt.ModelARGS, fp, sort_keys=True, indent=4)

    # also save the node module
    node_module_args = opt.ModelARGS['node_module']
    node_module_path = os.path.join(epoch_path, "node_module.pth.tar")
    node_module = model_to_save.node_module
    torch.save(node_module.state_dict(), node_module_path)
    with open('{}/NODE_MODULE_config.json'.format(epoch_path), 'w') as fp: json.dump(node_module_args, fp, sort_keys=True, indent=4)
...
def get_presaved_embeddings_and_tensors(dataset_path):
    tensors_paths = []
    tensor_labels = []

    # list the folders in the dataset path
    exp_folders = os.listdir(dataset_path)

    for exp_folder in exp_folders:
        # list directories in exp_folder
        exp_folder_path = os.path.join(dataset_path, exp_folder)
        exp_subfolders = os.listdir(exp_folder_path)
        if(len(exp_subfolders) != 1):
            raise ValueError("There should be only one subfolder in the experiment folder, but found {}\nExps: {}".format(exp_subfolders, exp_folder_path))
        tensor_folder_name = exp_subfolders[0]
        seed = extract_seed(tensor_folder_name)
        tensor_folder_path = os.path.join(exp_folder_path, tensor_folder_name, 'X.pt')

        # check path exists, other wise throw error
        if not os.path.exists(tensor_folder_path):
            raise ValueError(f"Path {tensor_folder_path} does not exist")

        tensors_paths.append(tensor_folder_path)
        tensor_labels.append(f"{exp_folder} | s{seed}")

    return tensors_paths, tensor_labels
